...
async def show_tasks(message: Message):
    ...
    await message.answer("<b>📋 SIZNING VAZIFALARINGIZ\n\n</b>", reply_markup=user_tasks_keyboard(tasks))
...
async def select_task(callback: CallbackQuery, state:FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"{task[1]}\n\n"
        f"{'✅ Bu vazifa allaqachon bajarilgan.' if completed else '<i>📎 Bajarilgan ishni tasdiqlovchi fayl yuboring.</i>'}",
...
@router.callback_query(F.data.startswith("recomplete_"))
...
async def process_task_completion(message: Message, state: FSMContext, bot:Bot):
    ...
    try:
        ...
        await message.answer(tasks_message, reply_markup=user_tasks_keyboard(tasks))
