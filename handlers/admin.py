...
from keyboards import (
    ...
    admin_admins_menu, admin_admins_list_menu, admin_roles_list_menu
)
...
async def admin_workers_callback(call: CallbackQuery, state: FSMContext):
    ...
    elif current_state == AddRoleStates.waiting_for_role_list.state:
        pass
...
@router.callback_query(F.data == "vazifalar_b")
async def admin_tasks(call: CallbackQuery, state: FSMContext):
    """Vazifalar bo'limi"""
    await call.message.edit_text(
        "<b>📋 VAZIFALAR BOSHQARUVI</b>\n\n"
        "Quyidagi amallardan birini tanlang:",
...
@router.callback_query(F.data=="admin_back_main", StateFilter(AddTaskStates.waiting_for_filial))
async def admin_tasks(call: CallbackQuery, state: FSMContext):
    """Vazifalar bo'limi"""
    await call.message.edit_text(
        "<b>📋 VAZIFALAR BOSHQARUVI</b>\n\n"
...
async def show_filial_statistics(callback: CallbackQuery):
    ...
    if not filial_stats:
        await callback.message.edit_text(
            ...
            "Bu filialda hali ishchilar yo'q yoki kecha hech qanday vazifa bajarilmagan.",
            reply_markup=select_filial_keyboard("stats_filial")
        )
        await callback.answer()
        return
    
    stats_message = format_filial_statistics(filial_stats, today)
    ...
    if len(stats_message) > 4000:
        ...
        await callback.message.answer(stats_message, reply_markup=select_filial_keyboard("stats_filial"))
...
async def ask_del_worker_check(message:Message, state:FSMContext):
    ...
    await message.answer(msg, reply_markup=is_check(message.text))
...
async def process_task_type(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"✅ Rol: {role[1]}\n"
        f"✅ Tur: {type_name}\n\n"
        f"<b>4️⃣ Vazifa matnini kiriting:</b>"
    )
...
async def list_tasks(callback: CallbackQuery):
    ...
    message = "<b>📋 VAZIFALAR RO'YXATI</b>\n\n"
    ...
    if len(message) > 4000:
        ...
        await callback.message.answer(message[4000:], reply_markup=admin_tasks_list_menu())
    else:
        await callback.message.edit_text(message, reply_markup=admin_tasks_list_menu())
...
async def role_list(message:Message, state:FSMContext):
    roles = db.get_all_roles()
    
    msg_rolels = "<b>🎭 Rollar ro'yxati:</b>\n\n"
    for role in roles:
        count = db.get_role_count(role[0])
        msg_rolels += f"🎭 {role[1]} - {count} ta\n"
    await message.answer(msg_rolels, reply_markup=admin_roles_list_menu())
    await state.set_state(AddRoleStates.waiting_for_role_list)


@router.callback_query(F.data=="add_role", AddRoleStates.waiting_for_role_list)
async def ask_role_name(call:Message, state:FSMContext):
    await call.message.delete()
    await call.message.answer("<b>🎭 Yangi Rolni kiriting:</b>")
    await state.set_state(AddRoleStates.waiting_for_role_name)


@router.message(F.text, AddRoleStates.waiting_for_role_name)
async def add_role(message:Message, state:Message):
    db.add_role(message.text)
    await state.clear()
    await message.answer("<b>✅ Yangi rol qo'shildi.</b>")
    await message.answer(
        "<b>🏠 ASOSIY MENYU</b>\n\n"
        "Quyidagilardan birini tanlang:",
        reply_markup=admin_main_menu())